/**
 * Enhanced Tradier Options Monitor
 * 
 * This is an enhanced version of tradierOptionsMonitor.js with pending order cancellation support.
 * 
 * To integrate:
 * 1. Backup your existing tradierOptionsMonitor.js
 * 2. Replace it with this file OR
 * 3. Copy the checkPendingOrderCancellation() method and add it to your existing monitor
 */
...
class TradierOptionsMonitor {
  ...
  async tick() {
    try {
      ...
      // NEW: Check for pending orders to cancel
      await this.checkPendingOrderCancellation(trades);
    } catch (error) {
  ...
  /**
   * NEW METHOD: Check for pending orders that should be cancelled
   */
  async checkPendingOrderCancellation(trades) {
    try {
      const settings = settingsService.getExchangeSettings(this.exchange);
      ...
      // Filter for trades with pending entry orders
      const pendingEntryTrades = trades.filter(trade => trade.status === 'pending_entry');
      
      for (const trade of pendingEntryTrades) {
        // Skip if already processing
        if (this.processing.has(`cancel-${trade.id}`)) {
          continue;
        }
        
        // Check if order should be cancelled
        const orderTime = new Date(trade.created_at);
        const shouldCancel = this.shouldCancelOrder(orderTime, cancelAfter, settings);
        ...
        // Cancel the order
        this.processing.add(`cancel-${trade.id}`);
        try {
          logger.info(`[${this.exchange}] Cancelling pending entry order for ${trade.option_symbol} - exceeded ${cancelAfter} timeout`);
          
          await this.cancelEntryOrder(trade);
          
          // Update trade status
          await updateOptionTrade(trade.id, {
            status: 'cancelled_timeout',
            notes: `Auto-cancelled after ${cancelAfter} (pending order management)`,
          });
          
          logger.info(`[${this.exchange}] Successfully cancelled pending order for ${trade.option_symbol}`);
        } catch (error) {
          logger.error(`[${this.exchange}] Failed to cancel pending order for ${trade.option_symbol}`, error);
        } finally {
          this.processing.delete(`cancel-${trade.id}`);
        }
      }
    } catch (error) {
      logger.warn(`[${this.exchange}] Pending order cancellation check failed`, error);
    }
  }

  /**
   * NEW METHOD: Determine if an order should be cancelled
   */
  shouldCancelOrder(orderTime, cancelAfter, settings) {
    const now = new Date();
    
    // Handle "before_session" mode
    if (cancelAfter === 'before_session') {
      return this.shouldCancelBeforeSession(settings);
    }
  ...
  /**
   * NEW METHOD: Check if we're approaching session end
   */
  shouldCancelBeforeSession(settings) {
  ...
  /**
   * NEW METHOD: Parse timeout string to milliseconds
   */
  ...
  /**
   * NEW METHOD: Cancel entry order for a trade
   */
  async cancelEntryOrder(trade) {
    try {
      // Cancel the order via Tradier API
      await this.api.cancelOrder(trade.underlying_symbol, trade.entry_order_id);
      logger.info(`Cancelled Tradier order ${trade.entry_order_id} for ${trade.option_symbol}`);
    } catch (error) {
      logger.error(`Failed to cancel Tradier order ${trade.entry_order_id}`, error);
      throw error;
    }
  }
